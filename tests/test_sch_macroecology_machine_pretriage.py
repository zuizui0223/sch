...
def test_machine_pretriage_covers_all_current_primary_includes():
    rows, receipt = _build()
    assert len(rows) == 153
    assert receipt["n_current_primary_includes"] == 153
    assert len({row["record_id"] for row in rows}) == 153
