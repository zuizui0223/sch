...
DENOMINATOR = 868
LATEST = "SCH_PRISMA_V2_SCREENING_DECISIONS_V25_ESTIMAND_PRIORITY_BATCH2.csv"
...
def _decision_files() -> list[Path]:
    ...
    assert [_version(path) for path in files] == list(range(1, 26))
...
def test_batch2_batch3_and_batch4_title_abstract_are_closed_without_double_screening() -> None:
    ...
    assert len(rows) == 418
    assert ta["RETAIN_FULLTEXT"] == 289
    assert ta["EXCLUDE"] == 129
    assert DENOMINATOR - len(rows) == 450
...
def test_current_evidence_lanes_keep_strict_at_two_under_v20_ta_only_progress() -> None:
    ...
    assert lanes["DIRECTIONAL_OR_NEAR_PASS"] == 107
