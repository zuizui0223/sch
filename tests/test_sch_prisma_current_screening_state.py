...
DECISIONS_V1 = ROOT / "empirical" / "prisma" / "SCH_PRISMA_V2_SCREENING_DECISIONS_V1.csv"
DECISIONS_V2 = ROOT / "empirical" / "prisma" / "SCH_PRISMA_V2_SCREENING_DECISIONS_V2_FULLTEXT_ADDITIONS.csv"
DENOMINATOR = 868
...
def _rows() -> list[dict[str, str]]:
    ...
    merged = {row["record_id"]: row for row in _read(DECISIONS_V1)}
    for update in _read(DECISIONS_V2):
...
def test_current_fulltext_state_is_fifteen_primary_includes_three_excludes() -> None:
    rows = _rows()
    ft = Counter(row["screen_fulltext"] or "UNSCREENED" for row in rows if row["screen_title_abstract"] == "RETAIN_FULLTEXT")
    assert ft["INCLUDE"] == 15
    assert ft["EXCLUDE"] == 3
    assert ft["UNSCREENED"] == 13
    reasons = Counter(row["screen_fulltext_reason"] for row in rows if row["screen_fulltext"] == "EXCLUDE")
    assert reasons["FT_REVIEW_ONLY_NO_PRIMARY_ROLE"] == 2
    assert reasons["FT_NO_DECLARED_FLORAL_COORDINATE"] == 1
...
def test_current_evidence_lanes_keep_one_strict_anchor() -> None:
    ...
    assert lanes["DIRECTIONAL_OR_NEAR_PASS"] == 14
    assert lanes["EVOLUTIONARY_OUTCOME"] == 5
...
def test_decision_provenance_remains_explicit_after_fulltext_additions() -> None:
    rows = _rows()
    sources = Counter(row["decision_source"] for row in rows)
    assert sources["PRIOR_SOURCE_ADJUDICATION_FULLTEXT_2026-08-30"] == 8
    assert sources["ASSISTED_SOURCE_VERIFIED_PRIMARY_SCREEN_2026-08-30"] == 25
    assert sources["SOURCE_VERIFIED_FULLTEXT_2026-08-30"] == 7
