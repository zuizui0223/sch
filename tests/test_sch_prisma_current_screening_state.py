...
DENOMINATOR = 868
LATEST = "SCH_PRISMA_V2_SCREENING_DECISIONS_V43_TA3_BATCH_F.csv"
...
def _decision_files() -> list[Path]:
    ...
    assert [_version(path) for path in files] == list(range(1, 44))
...
def test_batch2_batch3_and_batch4_title_abstract_are_closed_without_double_screening() -> None:
    ...
    assert len(rows) == 633
    assert ta["RETAIN_FULLTEXT"] == 412
    assert ta["EXCLUDE"] == 221
    assert DENOMINATOR - len(rows) == 235
...
def test_v19_closes_v18_fulltexts_and_v20_opens_only_new_batch4_fulltexts() -> None:
    ...
    retained_ta_ids = set().union(*[
        ...
        for version in (20, 21, 23, 25, 27, 33, 38, 39, 40, 41, 42, 43)
    ])
