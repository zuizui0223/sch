...
DENOMINATOR = 868
LATEST = "SCH_PRISMA_V2_SCREENING_DECISIONS_V14_BATCH3_HIGH_INFORMATION.csv"
...
def _decision_files() -> list[Path]:
    ...
    assert [_version(path) for path in files] == list(range(1, 15))
...
def test_batch2_title_abstract_remains_closed_without_double_screening_prior_anchor() -> None:
    ...
    before_v12_ids = {row["record_id"] for row in _merge(files[:-3])}
...
def test_v12_v13_and_v14_provenance_remain_stage_specific_in_raw_overlays() -> None:
    files = _decision_files()
    v12 = _read(files[-3])
    v13 = _read(files[-2])
    v14 = _read(files[-1])
    ...
    assert len(v14) == 33
    assert {row["decision_source"] for row in v14} == {
        "SOURCE_VERIFIED_BATCH3_HIGH_INFORMATION_TA_2026-08-31"
    }
    ...
    assert merged_sources["SOURCE_VERIFIED_BATCH3_HIGH_INFORMATION_TA_2026-08-31"] == 33
