...
def _decision_files() -> list[Path]:
    ...
    assert [_version(path) for path in files] == list(range(1, 20))
...
def test_batch2_and_batch3_are_closed_and_batch4_high_information_is_screened() -> None:
    rows = _rows()
    assert all(
    ...
    assert {"SCHPRISMA-000329", "SCHPRISMA-000339"}.isdisjoint(ids)
...
def test_v19_closes_the_v18_fulltext_queue() -> None:
