...
def test_macro_candidate_universe_keeps_every_current_fulltext_include():
    ...
    assert receipt["n_primary_candidates"] == 148
...
def test_macro_candidate_universe_does_not_filter_by_existing_evidence_lane():
    ...
    assert lane_counts["DIRECTIONAL_OR_NEAR_PASS"] == 133
    assert lane_counts["EVOLUTIONARY_OUTCOME"] == 41
...
def test_macro_candidate_universe_preserves_existing_geography_without_using_it_as_eligibility():
    ...
    assert geo["positive_geographic_contrast_records"] == 31
    assert geo["positive_receiver_contrast_records"] == 27
    assert geo["joint_positive_records"] == 25
    assert len(rows) == 148
