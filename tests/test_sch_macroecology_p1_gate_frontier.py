...
V34_BATCH_A = ROOT / "data" / "SCH_MACROECOLOGY_DESIGN_RECODE_V34_TA2_BATCH_A.csv"
V36_BATCH_C = ROOT / "data" / "SCH_MACROECOLOGY_DESIGN_RECODE_V36_TA2_BATCH_C.csv"
...
def _build():
    ...
    return mod.build(
        ...
        PRISMA,
        MACHINE_SCRIPT,
        [BATCH1, BATCH2, P1_REMAINDER, V26_HOLDOUT, V28_BATCH_A, V29_BATCH_B, V30_BATCH_C, V31_BATCH_D, V32_BATCH_E, V34_BATCH_A, V36_BATCH_C],
    )


def test_all_machine_p1_records_are_manually_gated():
    built = _build()
    assert built["n_machine_p1_records"] == 74
    assert built["n_p1_manual_gated"] == 74
...
def test_p1_gate_recovers_bounded_h1_candidate_frontier():
    ...
    assert built["geometry_eligibility_counts"] == {
        "BOUNDARY_BENEFIT_COST_COUPLED": 6,
        "ELIGIBLE_BOUNDED_COORDINATE": 28,
        "ELIGIBLE_SAME_COORDINATE": 11,
        "INELIGIBLE_MULTIVARIATE_UNRESOLVED": 6,
        "INELIGIBLE_NO_TWO_FUNCTION_GEOMETRY": 20,
        "UNRESOLVED_SOURCE": 3,
    }
    assert built["n_h1_geometry_candidate_records"] == 39
    assert built["n_h2_context_candidate_records"] == 31


def test_p1_gate_tracks_axis_decomposition_before_outcomes():
    built = _build()
    assert built["n_trait_axis_split_required_records"] == 34
    assert built["n_single_axis_records"] == 19
