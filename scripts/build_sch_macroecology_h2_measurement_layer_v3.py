...
def build(
    ...
    for row in measurements:
        ...
        referenced_objects = _split_ids(row["source_object_ids"])
        unknown_objects = sorted(set(referenced_objects) - source_object_set)
        if unknown_objects:
            raise ValueError(
        ...
        referenced_cases = _split_ids(row["case_ids"])
    ...
    return {
        ...
        "status": "H2_MEASUREMENT_LAYER_V3_GYMNADENIA_A2_MATERIALIZED",
        "claim_ceiling": [
            ...
            "mediated_component_point_estimates_without_contrast_uncertainty_are_not_local_geometry_cases",
