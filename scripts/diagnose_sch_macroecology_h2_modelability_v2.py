...
def build(
    ...
    reasons = []
    if len(all_clusters) < MIN_LAYER_CLUSTERS:
        reasons.append(
            f"only_{len(all_clusters)}_independent_clusters_have_materialized_local_cases"
        )
    if not performance_ready:
        ...
        reasons.append(
            f"only_{len(performance_repeated)}_plant_performance_axes_have_two_or_more_local_cases"
        )
    if not role_ready:
        reasons.append(
            f"role_behavior_layer_has_{len(role_behavior)}_cases_across_"
            f"{len(role_axis_counts)}_axes_and_{len(role_clusters)}_clusters"
        )
    reasons.extend([
        "plant_performance_and_role_behavior_are_different_estimands",
        "fixed_role_source_objects_remain_partly_unmaterialized",
    ])
