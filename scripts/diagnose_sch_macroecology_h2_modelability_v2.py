...
def build(
    ...
    performance_case_ids: set[str] = set()
    for row in measurements:
        for case_id in _split_ids(row["case_ids"]):
            if case_id in performance_case_ids:
                raise ValueError(f"duplicate measurement case ID: {case_id}")
            performance_case_ids.add(case_id)

    all_case_ids = set(case_ids)
    if not performance_case_ids <= all_case_ids:
        missing = sorted(performance_case_ids - all_case_ids)
        raise ValueError("measurement registry references missing H2 cases: " + ", ".join(missing))
    ...
    return {
        "analysis": "sch_macroecology_h2_modelability_gate_v2",
        "project_gates": {
        ...
        "permitted_now": [
            "treatment_group_net_selection_comparisons",
            "source_resolved_within_axis_context_descriptions",
            "within_source treatment or cultivar contrasts",
            "mechanistic change_type synthesis",
            "continued source_object recovery",
        ],
        "not_permitted_as_primary_now": [
            "mixed_effects_context_switch_model",
            "combined plant_performance plus role_behavior regression",
            "context_switch prevalence estimate",
            "treating treatment_group net selection as agent_specific local geometry",
        ],
        "reasons": [
            "only_5_independent_clusters_have_materialized_local_cases",
            "plant_performance_layer_has_11_cases_but_only_4_axes_and_3_clusters",
            "only_3_plant_performance_axes_have_two_or_more_local_cases",
            ...
            "Gentiana_Pedicularis_and_Primula_source_objects_remain_unmaterialized",
        ],
        "status": "H2_MODELABILITY_GATE_V2_GYMNADENIA_ADDED_FAIL_CLOSED",
    }
