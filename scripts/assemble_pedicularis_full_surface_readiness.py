...
EXPECTED = {
    ...
    "g": {
        "schema": "SCH_PEDICULARIS_ANTAGONIST_WEIGHT_V1",
        "status": "PEDICULARIS_ANTAGONIST_WEIGHT_VALIDATED",
    },
}
...
def assemble(z_receipt: dict, p_receipt: dict, g_receipt: dict) -> dict:
    ...
    return {
        ...
        "checks": checks,
        "source_receipts": {
            "z": {"schema": z_receipt.get("receipt_schema_version"), "status": z_receipt.get("status")},
            "p": {"schema": p_receipt.get("receipt_schema_version"), "status": p_receipt.get("status")},
            "g": {"schema": g_receipt.get("receipt_schema_version"), "status": g_receipt.get("status")},
        },
        "status": "PEDICULARIS_FULL_SURFACE_READY" if ready else "PEDICULARIS_FULL_SURFACE_NOT_READY",
        "unlocked_next_step": ">=5 realized z levels x P0/P1 x G0/G1" if ready else None,
        "claim_ceiling": "execution_readiness_only_not_causal_compromise",
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="Assemble fail-closed Pedicularis z/P/G readiness receipt")
